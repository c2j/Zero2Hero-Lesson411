...
//
  ...
  describe("Transactions", function () {
    ...
    it("Should fail if sender and transfer to are same", async function () {
      const { hardhatToken, owner, addr1 } = await loadFixture(deployTokenFixture);
      ...
      await expect(
        hardhatToken.connect(addr1).transfer(addr1.address, 1)
      ).to.be.revertedWith("Could not be same account");
